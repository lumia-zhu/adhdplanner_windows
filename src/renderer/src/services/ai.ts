...
function buildStuckChatSystemPrompt(context: StuckChatContext): string {
  ...
  return (
    ...
    '- 指代必须清楚：凡是提到“第一步”“这一步”“当前步骤”“回来点”“轻入口”，必须同时带上任务名或当前步骤名，例如“「任务名」的「当前步骤」”或“回到「任务名」”。不要裸说“这个第一步/破冰第一步/超轻第一步”。\n' +
...
- 如果用户问“不同心情/状态的日子，任务节奏有什么不同”，证据优先级必须是：1）每日心情记录；2）对应日期的完成率、任务数量、专注时长、任务推进；3）卡顿/中断只能作为辅助指标；4）记忆只能补充，不能替代当前周心情记录。
- 回答这类问题时，第一段必须点明至少 2 个有心情记录的日期或心情档位；不要用“状态不好、有情绪卡顿的两天”这种把心情和卡顿混在一起的说法。
...
- 如果本轮结构化调度说明用户在问“不同心情/状态记录下的任务节奏”，只有能补充状态应对方式的记忆才可选择；不要选择会替代当前每日心情记录的记忆
