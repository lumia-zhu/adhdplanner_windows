...
3. **看图聊天邀请**（1 句，必须与第二段之间有空行）：不要替用户解释为什么会这样；只邀请用户先看左边图表，例如“你可以先看看左边的图，哪个地方有点意思？也可以点下面的方向，我陪你一起看。”这一段同样必须和上一段之间留有空行。
...
- 第二步（用户消息包含“用户选择了分析角度：XXX”时）必须不输出 SUGGESTIONS 注释，也不要在正文里说“可以聊聊这几个方向”
...
3. **看图聊天邀请**（1 句）：不要替用户解释跨天规律；只邀请用户先看左边图表，例如“你可以先看看左边的图，哪个地方有点意思？也可以点下面的方向，我陪你一起看。”
