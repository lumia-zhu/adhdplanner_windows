...
function formatStuckCategoryGuide(category: StuckCategory): string {
  switch (category) {
    case 'task_understanding':
      return '分类方向：任务理解。首轮帮用户说出任务里不清楚的点，例如标准、材料来源、下一步或完成判断。问题保持开放；首轮不要举例，也不要输出“比如……”句子。第二轮根据当前任务生成一个“先确认什么”的动作，例子可以是确认标准、找信息入口、写下判断依据，但不要固定套用。'
    case 'task_load':
      return '分类方向：任务负荷。首轮帮用户回看任务是哪里开始变大、变复杂。问题保持开放；首轮不要举例，也不要输出“比如……”句子。第二轮根据当前任务生成一个最小可进入块，例子可以是只看一个文件、只写一句占位、只处理一个材料，但不适合时必须换成更贴合任务的动作。'
    case 'attention':
      return '分类方向：注意力。首轮不要责备分心，帮用户回看注意力被带走前发生了什么。问题保持开放；首轮不要举例，也不要输出“比如……”句子。第二轮根据具体分心入口和当前任务生成一个复位动作，例子可以是关掉入口、保留任务页面、回到任务停留很短时间，但不要硬套。'
    case 'emotion_motivation':
      return '分类方向：情绪/动力。首轮先接住心情不好、厌学、焦虑、低落、累、烦、抗拒，不急着拉回原任务。情绪类要走三段式：第一段让用户描述此刻的心情、身体感受或说不上来的阻力；第二段轻问这种心情大概从哪里来，不预设一定发生了某件事；第三段才复述听到的情绪来源，并给一个很轻的建议。前两段都不要给建议，不要举例，也不要输出“比如……”句子。第三段不要强行拉回原任务，可以建议休息、换到当天一个具体低阻力任务，或留下当前任务的具体回来点。'
    case 'context_conflict':
      return '分类方向：情境事务冲突。首轮承认现实事务会占用注意力，帮用户说出还有什么事在抢位置。问题保持开放；首轮不要举例，也不要输出“比如……”句子。第二轮根据现实事务和当前任务生成临时安排，例如先处理现实事务、先补资源入口、留下回来点，但必须以用户具体情境为准。'
  }
}
...
function formatStuckResponseModeGuide(mode: StuckResponseMode): string {
  switch (mode) {
    ...
    case 'emotion_elaboration':
      return '回应模式：情绪展开。三段式处理：第一轮不要二选一，不要给建议，只问“这会儿的 **心情** 是什么样的？可以随便描述一点。” 用户第一次回复心情后，第二轮仍然不要给建议，只追问“这种心情大概是从哪里来的？可以只说一点点。” 用户第二次补充来源后，再分流给轻量建议：累/困/exhausted/overwhelmed → 承认透支并建议短休息或降低刺激；没意义/pointless/做了也没用 → 承认意义感低，再建议换任务、连接长期目标或换一种低负担做法；无聊/没兴趣 → 建议换媒介、背景音乐或完成后奖励；烦/抗拒/害怕被问/刚发生了不舒服的事 → 不强推，建议留下回来点、短暂停下或做一个低触发动作。'
...
function buildStuckChatSystemPrompt(context: StuckChatContext): string {
  ...
  return (
    ...
    '5. 非情绪类第二轮不要继续提问，只给 1 个主建议；情绪类在用户第一次描述心情后，要先追问情绪来源，用户第二次补充后才给建议。建议要尽量绑定当前任务、当前步骤、当天任务名或历史模式，不要泛泛说“拆小一点/休息一下”。\n' +
    ...
    '轮次规则：\n' +
    '- 如果用户消息包含“【首轮卡住反思】”：先看回应模式。direct_action 直接给允许和回来点，不问问题；emotion_elaboration 只问“这会儿的 **心情** 是什么样的？可以随便描述一点。”；reflective_question 才问一个短开放问题。\n' +
    '- 如果用户消息包含“【情绪来源追问】”：只问“这种心情大概是从哪里来的？可以只说一点点。”，不要给建议、不要分析、不要举例。\n' +
    '- 首轮如果是开放问题，问题要单独成段，并且问题中必须加粗 1 个短的回答焦点；首轮到这里结束，不要再补充例子、不要输出“比如……”句子。\n' +
    '- 如果用户已经回复了非情绪类问题，或已经第二次补充了情绪来源：输出建议回复。先用 1 句复述你听到的模式，再给 1 个低压力建议；不要用问句结尾，不要要求用户马上选择。\n' +
...
- 开场必须用空行分成 3 个短段落：问候 / 一个证据锚点 / 一个隐藏行为现象；三段之间每处都必须有**且只有一个**空行（即两个换行符），不能多于一个空行，多余空行会造成段落间距不一致；首次回复不要问用户问题，不要展开建议
...
1. **一句简短问候**（≤ 20 字），语气轻松自然、像朋友打招呼。当前时段是**${timeOfDay}**，问候语必须与此一致（${timeOfDay}好 / 嗨～等），每次措辞不同。${preferredName ? `用户的称呼是「${preferredName}」，问候语里必须带上这个称呼，例如"${timeOfDay}好呀，${preferredName}，来看看${dayRefFirst}的情况。"` : '示例："${timeOfDay}好呀，来看看${dayRefFirst}的情况。"'}
   不要用"您好"这种正式称呼，保持朋友感。
2. **证据锚点**（只写 1 段，不拆两段）：图表引用放在这段最前面，然后在同一段里结合具体数字或事实夸用户，夸的时候必须带 emoji（💪 🎉 ✨ 🌟 等，选1个自然的），不要说空洞的"挺棒的/很好/不错"这类无数据支撑的句子。例如："【chart:metrics】今天专注了 **134 分钟**，任务全搞定了 💪"。不要把夸和洞察拆成两段，不复述图表里已经明摆着的数字。这一段结束后**必须空一行**再写第三段。
3. **隐藏行为现象**（1 句，必须与第二段之间有空行）：根据证据锚点指出一个用户平时不容易觉察、但和任务管理有关的现象。这个现象必须和上一段证据接得上；只讲一个现象，不要下结论。这一段同样必须和上一段之间留有空行。
4. 首次回复不要输出问题，也不要展开第一步、卡顿、记忆、历史承诺等细节；这些只能放到下方 3 个 Tag 中，等用户点击后再展开。

开场不要使用 1️⃣ / 2️⃣ 编号；不要用“要不要一起看看数据”作为固定收尾，因为用户已经在反思页里了。
...
第二步的目标是等待用户补充上下文，**不要输出 SUGGESTIONS 注释，不要生成新的 3 个 Tag**。等用户回答这个上下文问题后，第三步再给新的探索方向。
...
## Tag 方向（必须遵守）
只有开场回复和第三步“用户回答上下文后”的回复末尾需要附带探索方向，格式为 HTML 注释：
<!--SUGGESTIONS:["一直没碰的任务","和昨天的差别","比较活跃的时段"]-->

规则：
- 开场和第三步每次必须生成 **正好 3 个**Tag
- 第二步（用户消息包含“用户选择了分析角度：XXX”时）必须不输出 SUGGESTIONS 注释，也不要在正文里说“几个或许值得探索的方向”
...
- 直接开始，不自我介绍
- 开场只描述整体模式，不挑单个任务对比
- 开场第一条消息禁止提问，必须把可继续探索的方向交给 3 个分析标签
...
- 下一步引导要低压力，用"如果想继续看，可以先看..."，不要用"应该/必须/建议你分析"
- 开场问候单独一行；后面的数据洞察再用短段落呈现
- 开场必须用空行分成 3 个短段落：问候 / 一个周证据锚点 / 一个跨天隐藏行为现象；首次回复不要问用户问题，不要展开建议
- 用户回答后的策略回复也必须分段：承接用户回答 / 图表或数据对照 / 一个具体低压力策略 / 一句鼓励或轻总结；每段之间留空行
...
1. **一句简短问候**（≤ 15 字），语气轻松自然、像朋友打招呼。当前时段是**${wTimeOfDay}**，问候语必须与此一致（${wTimeOfDay}好 / 嗨～等），每次措辞不同。示例：
   ...
   - "这周辛苦啦，来看看数据"
   不要用"您好"这种正式称呼，保持朋友感。
2. **周证据锚点**（1 句为主，最多 2 句）：先引用 1 个最关键周图表或行为记录，但不要复述图表里已经明摆着的数字；只用它作为后面洞察的依据。
3. **跨天隐藏行为现象**（1 句）：根据证据锚点指出一个用户平时不容易觉察、但和任务管理有关的跨天现象。这个现象必须和上一段证据接得上；只讲一个现象，不要下结论。
4. 首次回复不要输出问题，也不要展开单个任务、卡顿、记忆、历史承诺等细节；这些只能放到下方 3 个 Tag 中，等用户点击后再展开。
