...
- 如果没有心情记录，但用户主动提到情绪，可以问：“如果只说一点点，这种不太好更像是累、烦，还是被任务压住了？”或“这种感觉大概是从开始前就有，还是做着做着冒出来的？”
...
- 推荐问题： “这次哪个小动作，值得下次继续用？” “下次做类似任务时，可以先做哪一小步？” “刚才让任务动起来的地方，哪一点最省力？” “回想那段比较顺的时候，哪些条件帮你跟上了？”
- 禁止问题： “为什么卡住？” “你当时为什么没有继续？” “你是下意识还是特意选的？” “你是怎么想到这样拆的？” “你觉得今天最大的收获是什么？” “平静的状态是不是帮你更顺？”
`
    : ''
...
- 如果没有心情记录，但用户主动提到情绪，可以问：“如果只说一点点，这种不太好更像是累、烦，还是被任务压住了？”或“这种感觉更像是某几天开始前就有，还是做着做着冒出来的？”
...
- 少用“入口”这类抽象词，改说“先做哪一小步”“哪个做法值得继续用”“怎么更容易开始”。
- 推荐问题： “这周哪个小动作，值得下周继续用？” “下周做类似任务时，可以先做哪一小步？” “让任务动起来的地方，哪一点最省力？” “哪些天的条件更容易让你开始？”
- 禁止问题： “为什么卡住？” “你当时为什么没有继续？” “你是下意识还是特意选的？” “你是怎么想到这样拆的？” “你觉得这周最大的收获是什么？” “平静的状态是不是帮你更顺？”
`
    : ''
  const structuredStepNote = reflectionStyle === 'free'
    ? ''
...
- 优先使用这些类型：
  ...
  - 情绪 × 行为观察类（仅当一周内有心情记录、用户主动提到状态，或当前主线涉及开始困难/卡顿/节奏变化时使用）：周视图必须做跨天比较，例如「哪些状态下更容易开始？」「状态不同的天任务怎么变了？」「状态低的天卡在哪里？」「哪些天状态和完成度不一致？」；不要把一天粒度的心情绑定到某个小时或单个任务瞬间。
- 避免单纯结果：「完成率100%」「15分钟专注」「没有待办」
- 避免图表入口：「指标卡片」「任务用时分析」「电脑活动图」
- 避免已经下结论：「任务切得刚好」「完成得很顺」「效率很好」
- 避免抽象或研究感表达：「比平时顺在哪里」「开始前少了什么阻力」「完成率背后的计划」「任务大小合不合适」「时间状态匹配」「策略复用」
