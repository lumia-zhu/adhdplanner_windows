...
function formatStuckCategoryGuide(category: StuckCategory): string {
  switch (category) {
    case 'task_understanding':
      return '分类方向：任务理解。首轮帮用户说出任务里不清楚的点，例如标准、材料来源、下一步或完成判断。问题保持开放，括号里最多给 2 个短例子。第二轮根据当前任务生成一个“先确认什么”的动作，例子可以是确认标准、找信息入口、写下判断依据，但不要固定套用。'
    case 'task_load':
      return '分类方向：任务负荷。首轮帮用户回看任务是哪里开始变大、变复杂。问题保持开放，括号里最多给 2 个短例子。第二轮根据当前任务生成一个最小可进入块，例子可以是只看一个文件、只写一句占位、只处理一个材料，但不适合时必须换成更贴合任务的动作。'
    case 'attention':
      return '分类方向：注意力。首轮不要责备分心，帮用户回看注意力被带走前发生了什么。问题保持开放，括号里最多给 2 个短例子。第二轮根据具体分心入口和当前任务生成一个复位动作，例子可以是关掉入口、保留任务页面、回到任务停留很短时间，但不要硬套。'
    case 'emotion_motivation':
      return '分类方向：情绪/动力。首轮先接住不想做、累、烦、抗拒，不急着拉回原任务。问题保持开放，括号里最多给 2 个短例子。第二轮不强行拉回原任务，可以建议休息、换到当天一个具体低阻力任务，或留下当前任务的具体回来点。'
    case 'context_conflict':
      return '分类方向：情境事务冲突。首轮承认现实事务会占用注意力，帮用户说出还有什么事在抢位置。问题保持开放，括号里最多给 2 个短例子。第二轮根据现实事务和当前任务生成临时安排，例如先处理现实事务、先补资源入口、留下回来点，但必须以用户具体情境为准。'
  }
}
...
function buildStuckChatSystemPrompt(context: StuckChatContext): string {
  ...
  return (
    ...
    '3. 问题必须白话、具体到“刚才那一刻”、开放式，不能是二选一/是或不是/多选题；括号里最多给 2 个短例子帮助用户知道能怎么答。\n' +
    ...
    '- 问题要开放，不要写成“是 A 还是 B”的二选一；如果给例子，要写成“比如 A，或 B”，并允许用户用自己的话说。\n' +
    ...
    '- 首轮的开放问题要单独成段，并且问题中必须加粗 1 个短的回答焦点；如果需要举例，必须用空行另起一段，用“比如……”给最多 2 个短例子，不要把问题和例子写在同一行或同一段。\n' +
