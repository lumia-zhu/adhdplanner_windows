...
`
  ...
  const structuredStepNote = reflectionStyle === 'free'
    ? '自由模式下，下面的“第二步/第三步”只作为可参考的回复素材，不是固定流程；优先遵守“下一步意图判断”。'
    : ''
...
export async function generateSuggestions(
  ...
  const countRule = reflectionStyle === 'free'
    ? '根据对话需要生成 0-3 个数据探索方向：如果上一轮主要是在问上下文、澄清、温和收束，或用户已经说不想继续，可以返回空数组；如果还有自然可聊的新方向，再返回 1-3 个。'
    : '通常生成 3 个数据探索方向；如果对话已经明显收束，或上一轮主要是在追问/澄清，也可以返回空数组。'

  const systemPrompt = `你是一个数据探索助手。根据对话上下文，${countRule}${modeHint}
要求：
- 方向是可点击的任务管理问题入口，不是结论、不是图表名、不是研究分类
- 每条 5-14 个中文字左右，必须是完整短句，不要半截话
- 分类只是候选池，不是配额；不要固定凑“有效经验类/卡点观察类/中性探索类”各 1 个。按当前 AI 回复和用户刚说的话，选择最自然、最不重复的 0-3 个方向
