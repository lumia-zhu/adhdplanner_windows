...
| 🧾 子任务清单 | 小组件默认只显示主任务名、一个清单项和进度，点击进度再展开 checklist，避免悬浮窗被子任务撑高 |
| ⏸️ 暂停续做 | 暂停中的任务在主界面和小组件中统一使用琥珀色继续入口，避免和绿色开始入口混淆 |
| 🖥️ 系统托盘 | 关闭窗口后隐藏到托盘，不退出程序 |
| 🚀 开机自启 | 托盘菜单可一键开启/关闭开机自动启动 |
| 💾 本地持久化 | 数据保存在本地 JSON 文件，重启不丢失 |
| 🙂 每日心情记录 | 以 5 级心情保存当天状态，界面会按日期自动轮换表情、天气、植物、月相等视觉主题，并在日反思中作为轻量状态背景展示 |
| 📊 日反思任务用时 | 任务用时排行默认只展示前 5 项，任务较多时可一键展开剩余内容，避免图表过长挤压后续分析 |
| 💬 反思回复精简 | AI 反思的策略回复会收敛为一个清楚动作，避免正文和引用块重复总结 |
| 🔎 反思开场引导 | AI 开场先用开心表情做具体肯定，再用更短的话邀请用户看左侧图表；底部 Tag 会根据当前对话和用户回复动态推荐，不再固定按三类各给 1 个 |
| 🧠 自由反思模式 | 可切换到更自然的反思聊天：底部使用标准探索方向 Tag，并支持一键换一批候选；前端会先判断用户意图，再让 AI 接住情绪、按深浅引导探索或提取可保留做法 |
...
- **2026-05-10**：
  ...
  - 优化 Widget「卡住了」面板的开场提问：会带上用户称呼，并改为“Hi [称呼]，你卡住的原因是？”，其中问题部分高亮展示。
  - 优化情绪类卡住支持：当用户输入“心情不好、厌学、焦虑、低落”等原因时，AI 会先问“这会儿的心情是什么样的？可以随便描述一点”，用户描述心情后再追问“这种心情大概是从哪里来的？可以只说一点点”，最后才结合回答给轻量建议。
  - 涉及文件：`src/renderer/src/components/ProfileSettings.tsx`、`src/renderer/src/components/StandbyWidget.tsx`、`src/renderer/src/components/WidgetView.tsx`、`src/renderer/src/types/index.ts`、`src/renderer/src/App.tsx`、`src/renderer/src/components/ReflectionView.tsx`、`src/renderer/src/services/ai.ts`、`docs/stuck-support-prompt.md`。
...
- **2026-05-04**：
  - 将反思对话的局部高亮升级为结构化输出，同时保留正文流式体验：AI 正文仍通过 `chatReflectionStream` 逐字显示；当模型在流式文本中输出隐藏的 `VISUAL_REF` 注释时，前端会立即解析并同步高亮左侧图表（注释不会展示给用户）。如果流式过程中没有成功触发高亮，回复完成后前端再把真实回复交给轻量结构化选择器，从 `availableVisualTargets`（指标卡、活动小时/时间段、任务条、应用条目）中选择 `visualFocus`。`visualFocus` 为 **0～3 个** `targetId`（数组）；并列提到多个应用（同一「应用使用时长」图）时可返回多项，其它场景仍以单项为主。结构化高亮失败时，会根据正文里的图表引用兜底高亮整张图表，不影响正文回复。
  - 优化局部高亮交互：结构化 `visualFocus` 不再额外显示为一个独立按钮，而是绑定到 AI 正文里的图表引用上（多条应用会合成 `multi-focus`，点击「📊 应用使用时长」可同时轻描多行）；AI 回复出现时会自动轻量提示一次，用户点击正文图表引用可再次触发对应局部高亮。高亮样式也从大面积色块改为细线、轻描边和底部提示线，减少对用户注意力的打扰。
  - 优化高亮可见性：AI 触发局部或整图高亮时，会显式滚动左侧数据可视化面板，并尽量把目标图表放到可视区域中间；如果目标已经处在舒适的中间区域，则不滚动，避免页面频繁跳动。滚动后会稍微延迟再开始闪烁，让用户更容易注意到被强调的主体。
  - 再次优化 AI 证据高亮的视觉表达：折线图不再绘制顶部橙色短线，避免被误认为图表数据；电脑活动分布里的小时提示改为空心框闪烁，不再用色块覆盖真实活跃度颜色，连续多个小时会合并成一个整体大框；任务条、应用条和指标卡统一改为约 5 秒的柔和闪烁提示。结构化回复落地后约 300ms 自动触发高亮，用户点击正文图表引用时也使用同样的短暂提示。
  - 新增反思对话的「单一类型重点」机制：同一时间左侧仍只保留一种高亮语义（例如要么是某时段、要么是并列多个应用），新的点击会替换旧高亮；AI 仍可引用整张图表（`chart:ID`），也可用 `focus` 引用触发左侧局部高亮。
  - 局部高亮第一版支持日视图中的电脑活动小时段、连续活动时间段、任务用时条、核心指标卡片和应用使用时长条目；如果 AI 给出的小时、任务名、指标 key 或应用名无法匹配，会自动退回到对应整张图表高亮，不会报错或让界面卡住。周视图暂时保持整图引用，避免“某天某小时”定位不准。
  ...
  - 优化反思 Tag 的命名边界：Tag 只展示“分析方向”，不提前暴露具体任务名、课程名、应用名或精确时段；例如「总被留到后面的学习任务」会改为「总被留到后面的任务」，具体是哪个任务等用户点击后再在正文里展开。
  - 小范围调整反思 Tag 候选词：将「停下来的是哪一步」收敛为「分析卡顿情况」，去掉「卡住后怎么继续的」「类似的一次卡住」，将「和前几天相比」改为「和前几天的区别」，并新增「比较集中的推进时间」「哪些环境更容易推进」两个分析方向。
  - 调整反思对话第二步体验：用户点击某个分析方向后，AI 只解释该方向、结合图表/行为记录追问 1 个开放式上下文问题，不再立刻显示新的“可以聊聊这几个方向”；等用户补充上下文后，第三步再承接回答、给低压力建议并重新提供 3 个新方向。前端也增加了兜底过滤，避免模型误输出 `SUGGESTIONS` 时在第二步显示按钮。
  - 优化任务用时图的时间显示：不再把 3分50秒、3分04秒都粗略显示成 `3 min`，改为 `3m 50s` 这类秒级格式；日视图任务用时和周视图任务排行保持一致，避免柱子长度和文字看起来矛盾。
  - 反思页右上角新增全屏按钮，位于关闭按钮左侧；用户可以在数据图表和 AI 对话之间需要更多空间时放大窗口，再次点击可还原到原来的反思窗口尺寸。放大状态下点击「结束反思」会和普通状态保持一致：先保存记忆，再恢复窗口并回到主界面。
  ...
  - 修正反思 AI 的图表引用规则：任务用时图只用于解释任务总耗时和排行；卡顿红点、任务发生时间段、卡住前后是否接上，都属于「电脑活动分布」图。AI 讨论卡顿或恢复时会优先引用 `chart:activity`，避免用户点到任务用时图却看不到对应卡顿位置；应用使用时长也补充为可点击图表（`chart:app-usage` / `chart:week-app-usage`），用于解释电脑活跃时段具体可能在用哪些前台应用。
  - 涉及文件：`src/renderer/src/services/ai.ts`、`src/renderer/src/components/ReflectionChat.tsx`、`src/renderer/src/components/ReflectionView.tsx`、`src/renderer/src/components/ActivityRhythmChart.tsx`、`src/renderer/src/components/InteractiveActivityHeatmap.tsx`、`src/renderer/src/components/TaskDurationChart.tsx`、`src/renderer/src/components/AppUsageRanking.tsx`。
