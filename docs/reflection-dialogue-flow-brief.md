...
当用户问“不同心情/状态的日子，任务节奏有什么不同”时，会额外加入约束：

- 先基于每日心情记录分组或逐日比较。
- 再看对应日期的完成率、任务数、专注时长、任务推进。
- 卡顿/中断只能作为任务节奏的辅助指标。
- 记忆只能补充，不能替代当前周的心情记录。
...
- 如果用户问的是当前心情记录和任务节奏，必须先回答当前心情数据；记忆只能在后面轻轻补充。
...
```text
可以看这个方向，但要先按这周每天记录的心情来对照，不把卡顿直接当成心情原因。

【chart:week-completion】比如 **周一** 记录为状态比较平稳，任务推进更完整；**周三** 记录为状态低，完成率和专注时长都更低；到 **周五** 状态较好时，任务推进又更连续一些。
