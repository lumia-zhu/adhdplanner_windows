...
当前 AI 反思以**自由反思模式**为默认实现。旧版三段式逻辑仍在代码中保留为兼容路径，但当前产品不再把用户限制在固定的“点击方向 → 回答问题 → 接收建议”流程里。

| 模式 | 默认状态 | 适合场景 |
|---|---|---|
| 自由反思 | 默认开启 | 用户可以直接输入、点 Tag、表达情绪或说不知道聊什么；AI 每轮判断该解释数据、接住情绪、澄清线索、给小实验还是收束。 |
| 三段式 | 兼容保留 | 仅作为旧版稳定流程的保留实现，当前 UI 不暴露切换入口。 |

旧版三段式不是一次性给用户一大段分析，而是一个循环式对话。当前自由反思保留了“开场 + 可选 Tag”的低负担入口，但不会强制后续都按这个循环推进：
...
自由反思模式保留开场和 Tag，但不会强制每轮都走“第二步/第三步”。AI 每轮先判断用户当前最需要什么：数据不够解释原因时先问 1 个上下文问题；用户已经说清楚原因时给 1 个低压力建议；用户回复模糊但有重要线索时再澄清 1 次；用户已经有发现或不想继续时温和收束；还需要探索时才展示新的 Tag。
...
| 轻入口类 | 给不知道聊什么或不想深聊的用户一个更轻的开始方式。 | 先看一个顺的地方？今天哪里需要变小？只收一个小发现？ |
...
| 分类 | 对应 Tag |
|---|---|
| 轻入口类 | 先看一个顺的地方？<br>今天有什么值得保留？<br>今天哪里需要变小？<br>只收一个小发现？ |
...
| 分类 | 对应 Tag |
|---|---|
| 轻入口类 | 先看这周顺的地方？<br>这周有什么值得保留？<br>这周哪里需要变小？<br>这周只收一个小发现？ |
...
新鲜感主要来自入口类型轮换，而不是夸张比喻。推荐用朴素、能直接理解的入口，例如“先看一个顺的地方？”“今天哪里需要变小？”“只收一个小发现？”。避免“游戏关卡”“侦探破案”“大脑天气”“挑战任务”“隐藏小胜利”这类容易显得表演化的表达，除非用户自己先使用类似语气。
...
```mermaid
flowchart TD
  aiReply["AI 回复正文"] --> suggestionCall["前端调用 generateSuggestions"]
  suggestionCall --> renderButtons["渲染成底部 Tag 按钮"]
...
当前主回复 AI 不再负责输出 `SUGGESTIONS` 隐藏注释。回复完成后，前端会把最近对话上下文交给 `generateSuggestions()`，由一个单独的结构化调用从标准 Tag 池里挑选候选方向。这样可以避免正文里混入控制文本，也更容易在自由反思里返回 0 个 Tag。
...
| 场景 | 是否生成 Tag |
|---|---|
| AI 开场 | 通常生成一组标准 Tag 候选，前端每次展示 3 个，可换一批 |
| 用户点击 Tag 后 | 如果本轮主要是在解释或轻问上下文，可以不生成 Tag |
| 用户继续补充后 | 按需要生成 0 个或一组标准 Tag 候选；若对话已收束可以不生成 |
| 自由反思追问、澄清或收束时 | 可以不生成 Tag |
| 对话自然收尾 | 可以不生成 Tag |
...
自由反思没有固定三步骨架。用户点 Tag 时，前端会告诉 AI“用户想顺着这个话题聊”，AI 每轮回复前应该先判断用户当前最需要的是看懂数据、补充上下文、继续澄清、获得建议、换个方向，还是结束反思。

自由模式使用“微技能选择器”，不是固定脚本。AI 每轮先判断用户状态，再只选择一个动作：接住情绪、肯定努力、换视角、提取做法、轻问一个问题、给方向选择、小实验或温和收束。

前端还会在自由模式下加入一层轻量对话管理。用户发送消息时，`ReflectionChat` 会先把这句话归到一个大致状态，例如选择方向、情绪首轮、情绪来源、困难变轻、无话可说、提取做法、主动问怎么办、想结束或开放补充。这个判断只作为隐藏指令发给 AI，用户不会看到。
...
回复完成后，前端会单独调用 `generateSuggestions()` 生成候选方向。用户实际看到的是正文和 3 个按钮，不会看到任何控制文本。
...
这一轮通常只解释方向并问一个必要问题，不急着给建议，也可以不生成新 Tag。
...
这样一轮就完成了：Tag 帮用户选方向，AI 问必要上下文；用户补充后，AI 把数据和真实情况合在一起给一个低压力建议。自由反思下，如果用户直接输入问题、表达情绪或想结束，也可以跳过这个顺序。

## 相关文件

| 文件 | 作用 |
|---|---|
| `src/renderer/src/services/ai.ts` | 生成每日反思和周反思 system prompt，定义自由反思调度、Tag 规则、图表引用和高亮边界。 |
| `src/renderer/src/components/ReflectionChat.tsx` | 展示 Tag 按钮，处理用户点击 Tag 和自由输入，并在自由模式下注入隐藏调度指令。 |
