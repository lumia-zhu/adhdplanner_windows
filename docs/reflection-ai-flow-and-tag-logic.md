...
| 场景 | 是否生成 Tag |
|---|---|
| AI 开场（默认三段式） | 通常生成 3 个 Tag，但动态选择，不固定三类配比 |
| 用户点击 Tag 后的第二步（默认三段式） | 不生成 Tag |
| 用户回答上下文后的第三步（默认三段式） | 通常生成 3 个新 Tag，但动态选择；若对话已收束可以不生成 |
| 自由反思模式 | 按需要生成 0-3 个 Tag；追问、澄清或收束时可以不生成 |
| 对话自然收尾 | 可以不生成 Tag |

## 弹性回应策略

三步循环是默认骨架；打开自由反思后，它不再是强制脚本。AI 每轮回复前，应该先判断用户当前最需要的是看懂数据、补充上下文、继续澄清、获得建议，还是结束反思。
