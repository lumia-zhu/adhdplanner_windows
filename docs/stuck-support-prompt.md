...
| `emotion_elaboration` | 心情不好、厌学、不想做、累、没兴趣、抗拒、焦虑、低落、没力气 | 三段式：先问心情状态，再追问情绪来源，最后才给轻量建议 |
...
如果粗分类是 `emotion_motivation`，进入情绪展开模式。情绪类走三段式：先让用户描述心情，再轻问这种心情大概从哪里来，最后才给轻量建议。

第一轮不要给建议，也不要二选一；目标是先给用户一个情绪出口，让用户描述此刻的心情、身体感受或说不上来的阻力，不预设一定发生了某件事。

```text
这会儿的 **心情** 是什么样的？可以随便描述一点。
```

第二轮仍然不要给建议，只轻问情绪来源：

```text
这种心情大概是从哪里来的？可以只说一点点。
```
...
| 烦、抗拒、害怕被问、刚发生了不舒服的事 | 不强推，建议留下回来点、短暂停下或做一个低触发动作 |
...
5. 非情绪类第二轮不要继续提问，只给 1 个主建议；情绪类在用户第一次描述心情后，要先追问情绪来源，用户第二次补充后才给建议。
