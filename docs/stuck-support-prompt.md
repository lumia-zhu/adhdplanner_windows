...
Stuck Support 的目标不是用户一说“卡住了”，AI 就立刻给建议，而是先帮助用户看见刚才为什么卡住，再把这个反思转成一个低压力、可执行的选择。
...
## 二、两轮式 Stuck Support Flow

```mermaid
flowchart LR
  A["用户点击：卡住了"] --> B["选择/输入卡住原因"]
  B --> C["系统分类 stuckCategory"]
  C --> D["第一轮：AI 接住 + 可选数据锚点 + 1 个开放问题"]
  D --> E["用户补充上下文"]
  E --> F["第二轮：AI 复述模式 + 1 个低压力建议"]
  F --> G["用户继续任务 / 暂停 / 换方式"]
```
...
## 十、和 `docs/stuck-reason-coding.md` 的关系
